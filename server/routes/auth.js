...
router.post("/signup", (req, res, next) => {
    ...
    // Use regex to validate the password format ------ Keep it simple para poder test el code 
        ...
        User.create({ email, password: hashedPassword, name })
          ...
          .catch((err) => {
            if(err instanceof mongoose.Error.ValidationError) {
                ...
                res.status(503).json({message: "Error encounter", err})
            }
          });
      })
      .catch((err) => {
...
// Verify the validity of the token - isAuthenticated
